...
async function downloadAudio(url) {
  ...
  if (res.status !== 200) throw new Error(`Download do áudio falhou: ${res.status}`)
...
async function transcribeAudio(audioData, mimeType = 'audio/ogg') {
  try {
    ...
    // 2. Montar multipart/form-data com form-data (garante Content-Type correto)
